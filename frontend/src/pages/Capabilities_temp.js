  ...
  return (
    <div>
      ...
      <Typography variant="body1" gutterBottom>
        Use this page to add capabilities, which are essentially the intersection between a component and an attribute.
      </Typography>
      {/* Search and Filter */}
          ...
          <TableHead>
            <TableRow>
              ...
              {attributes.map((attribute) => (
                <HeaderTableCell key={attribute.id}>
          ...
          <TableBody>
            {filteredComponents.map((component) => (
              <TableRow key={component.id}>
                <HeaderTableCell>
                  <Typography variant="h6" sx={{ fontSize: '1.1rem', color: theme.palette.primary.main, fontWeight: 'bold' }}>
                    {component.name}
                  </Typography>
                </HeaderTableCell>
                {attributes.map((attribute) => (
                  <StyledTableCell key={attribute.id}>
                    {capabilities
                      ...
                      .map((cap) => (
                        <Typography key={cap.id}>
                          {cap.name}
                        </Typography>
                      ))}
                    <EditIconButton onClick={() => handleOpen(component.id, attribute.id)}>
