...
export const createCapability = async (data) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/capabilities`, data, getAuthHeaders());
    return response.data;
  } catch (error) {
...
export const updateCapability = async (id, data) => {
  try {
    const response = await axios.put(`${API_BASE_URL}/capabilities/${id}`, data, getAuthHeaders());
    return response.data;
  } catch (error) {
