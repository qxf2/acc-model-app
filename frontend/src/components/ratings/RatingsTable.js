...
import { Box, Typography, IconButton, TableCell, TableRow, Select, MenuItem } from '@mui/material';
import { ExpandMore, ExpandLess, Edit } from '@mui/icons-material';

const RatingsTable = ({ components, attributes, expandedComponents, handleToggleExpand, capabilities, ratings, handleRatingChange, handleOpenModal, ratingOptions }) => (
  <>
        ...
        {expandedComponents[component.id] && capabilities
          ...
          .map((capability) => (
            <TableRow key={capability.id}>
              ...
              {attributes.map((attribute) => (
                <TableCell key={`${capability.id}-${attribute.id}`} style={{ position: 'relative' }}>
                  <Select
                    value={ratings[`${capability.id}-${attribute.id}`]?.rating || ''}
                  ...
                  <IconButton
                    ...
                    onClick={() => handleOpenModal(capability.id, attribute.id)}
                  >
                    <Edit />
                  </IconButton>
                </TableCell>
